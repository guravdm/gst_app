...
<%
	if (session.getAttribute("userId") != null) {
		...
		session.setAttribute("page", "poaddnew");
...
<%
	Connection con = DoConnection.getConnection();
		...
		PreparedStatement pz = con
				.prepareStatement("select * from material_master where po_status = 0");
		ResultSet rz = pz.executeQuery();
		while (rz.next()) {
			mtrlName += rz.getString(3) + ",";
		}
%>
<input type="hidden" value="<%=data%>" name="cliName" id="cliName" />
<input type="hidden" value="<%=mtrlName%>" name="mtrlame" id="mtrlame" />
<script>
	$(function() {
		var nameClnt = document.getElementById("cliName").value;
		var materialName = document.getElementById("mtrlame").value;
		//alert(materialName);
		// alert(name);
		var result = nameClnt.split(',');
		alert(result)
						...
						<div class="panel-body bg-light">
							...
							<div class="section">
								<label for="companyName" class="field prepend-icon"> <input
									...
									value="<%=data%>" class="gui-input" placeholder="Company Name">
							...
							<div class="section">
								<label for="useremail" class="field prepend-icon"> <input
									...
									class="gui-input" placeholder="Material Name"> <label
									for="material_name" class="field-icon"> <i
