...
<%
	if (session.getAttribute("userId") != null) {
		...
		session.setAttribute("page", "po-new");
...
<%
	Connection con = DoConnection.getConnection();
		String data = "", mtrlName = "";
		String ClntId = "";
		PreparedStatement p = con
				.prepareStatement("select * from client_master where id = ?");
		p.setString(1, request.getParameter("clientId"));
		ResultSet r = p.executeQuery();

		while (r.next()) {
			data += r.getString(3) + "";
			ClntId = r.getString(1);
		}

		PreparedStatement pz = con
				.prepareStatement("select * from material_master where client_id = ? and po_status = 0");
		pz.setString(1, request.getParameter("clientId"));
		// pz.setString(2, request.getParameter("clientId"));
		ResultSet rz = pz.executeQuery();
		// System.out.println(pz);
		while (rz.next()) {
			mtrlName += rz.getString(3) + ",";
		}
%>
<input type="hidden" value="<%=data%>" name="clntName" id="clntName" />
<input type="hidden" value="<%=mtrlName%>" name="mtrlame" id="mtrlame" />
<script>
	$(function() {
		var name = document.getElementById("clntName").value;
		var materialName = document.getElementById("mtrlame").value;
		//alert(materialName);
		// alert(name);
		var result = name.split(",");
		var resultMtrl = materialName.split(",");
		...
		<div class="tray tray-center">
			...
			<div class="admin-form theme-primary mw1000 center-block"
				...
				<div class="panel heading-border">
					<form method="post" action="./AddNewPoCntrl" autocomplete="off"
						id="admin-form">
						<div class="panel-body bg-light">
							...
							<input type="hidden"
								value="<%=request.getParameter("clientId")%>" name="custCode"
								id="custCode" />
							...
							<div class="section">
								<label for="companyName" class="field prepend-icon"> <input
									...
									id="companyName" autofocus value="<%=data%>" class="gui-input"
								...
								<select name="material_name" id="material_name"
									...
									<%
										PreparedStatement pt = con
													.prepareStatement("select * from material_master where client_id = ?");
											pt.setString(1, request.getParameter("clientId"));
											ResultSet rt = pt.executeQuery();
											// System.out.println(pt);
											while (rt.next()) {
												if (rt.getString("po_status").equalsIgnoreCase("0")) {
									%>
									<option value="<%=rt.getString(3)%>"><%=rt.getString(3)%></option>
									<%
										}
											}
									%>
							...
							<div class="section row" id="spy1">
								<div class="col-md-6">
									<label for="poNo" class="field prepend-icon"> <input
										type="text" name="poNo" id="poNo" class="gui-input"
										placeholder="PO Number"> <label for="poNo"
								...
								<div class="col-md-6">
									<label for="rate" class="field "> <input type="text"
										name="rate" id="rate" Rate"" class="gui-input" value="0"
										placeholder="Rate"> <label for="rate"
